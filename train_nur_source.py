""" copied from train.py, modified to train only on source domain
this is the pre-training before training a Siemese network for
domain adaptation.
"""
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if False:
        pretrained_weights = torch.load('trained_model/unet_beijing.pth.tar', map_location='cuda:0')
...
class Trainer(object):
    ...
    def training(self, epoch, train_loader):
        ...
        srbar = tqdm(train_loader)
        num_img_sr = len(train_loader)

        for i, data in enumerate(srbar):
            source_set = data
            ...
            srbar.set_description('Train loss: %.3f' % (train_loss / (i + 1)))
            self.writer.add_scalar('train/total_loss_iter', loss.item(), i + num_img_sr * epoch)

        self.writer.add_scalar('train/total_loss_epoch', train_loss, epoch)
        print('[Epoch: %d, numImages: %5d]' % (epoch, i * self.args.batch_size + srimg.data.shape[0]))
